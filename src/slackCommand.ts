...
import {
  setPhoneNumber,
  removeActivePhoneNumber
} from "./pagerduty/phoneNumberManagement";
...
async function handleCallMe(ctx: Koa.Context) {
  ...
  ctx.response.body = {
    ...
    text: `Your phone number +${ctx.phoneNumber[0]} ${
      ctx.phoneNumber[1]
    } is set in pagerduty`,
    attachments: [
      {
...
export async function handleSlackCommand(
  ...
  const userInfo: UserProfile = await fetchSlackUserProfile(
  ...
  if (ctx.phoneNumber) {
    ...
    if (handlers[command]) {
      ...
      ctx.response.body = `Command ${command} is not supported. Try one of these "${Object.keys(
        handlers
      ).join('" / "')}"`;
    }
  } else {
